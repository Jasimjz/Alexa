...
await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/Devil.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted : message.data, ptt: true})
        }
const array = ['Error','Nanban','Sed','pokko','Unstoppable','Zaynah','life','Rauf','Terror','Chodhyam','ikka','TMT','Oombi','Ghost', 'Entha','Nature','Bye','Jz','Magu','Alexa','baby','Ramesh','Rameez','Manikyam','Sound','Kunjatta','Malfoy','Waite','Vanam','Jasim','Dex','Enjoy','Gooth','Oomb','Bgm']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
       await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/' + a + '.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted: message.data, ptt: true})
...
const array = ['Hija','Clap','Wife','Love','Sed','Kiss','Food','Mol']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
